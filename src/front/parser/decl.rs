...
impl<'a> Parser<'a> {
    pub(crate) fn parse_type_specifier(&mut self) -> PResult<(CType, Span)> {
        // typedef-name ?
        {
            let t = self.ts.peek(0).map_err(|e| ParseError { msg: e.msg.clone(), span: e.span })?.clone();
        ...
        loop {
            ...
            match t0.kind {
                K::Signed => { let t = self.ts.bump().map_err(|e| ParseError { msg: e.msg, span: e.span })?; if signed.is_some() { return Err(ParseError { msg: "modificateur 'signed' en double".into(), span: t.span }); } signed = Some(true); first_span.get_or_insert(t.span); }
                K::Unsigned => { let t = self.ts.bump().map_err(|e| ParseError { msg: e.msg, span: e.span })?; if signed.is_some() { return Err(ParseError { msg: "modificateur de signe en double".into(), span: t.span }); } signed = Some(false); first_span.get_or_insert(t.span); }
                K::Long => {
                    let t = self.ts.bump().map_err(|e| ParseError { msg: e.msg, span: e.span })?;
                    if long_flag { return Err(ParseError { msg: "'long' en double".into(), span: t.span }); }
                    if short_flag { return Err(ParseError { msg: "'short' et 'long' ne se combinent pas".into(), span: t.span }); }
                    long_flag = true; first_span.get_or_insert(t.span);
                    // si base est déjà Double, on annotera plus tard (long double)
                }
                K::Short => {
                    let t = self.ts.bump().map_err(|e| ParseError { msg: e.msg, span: e.span })?;
                    if short_flag { return Err(ParseError { msg: "'short' en double".into(), span: t.span }); }
                    if long_flag { return Err(ParseError { msg: "'short' et 'long' ne se combinent pas".into(), span: t.span }); }
                    short_flag = true; first_span.get_or_insert(t.span);
                }
                K::Int => {
                    let t = self.ts.bump().map_err(|e| ParseError { msg: e.msg, span: e.span })?;
                    if base.is_some() { return Err(ParseError { msg: "spécificateur de type en double".into(), span: t.span }); }
                ...
                K::Char => {
                    let t = self.ts.bump().map_err(|e| ParseError { msg: e.msg, span: e.span })?;
                    if base.is_some() { return Err(ParseError { msg: "spécificateur de type en double".into(), span: t.span }); }
                    if long_flag || short_flag { return Err(ParseError { msg: "'char' ne se combine pas avec short/long".into(), span: t.span }); }
                    base = Some((CType::Char { signed }, t.span));
                    first_span.get_or_insert(t.span);
                }
                K::Void => {
                    let t = self.ts.bump().map_err(|e| ParseError { msg: e.msg, span: e.span })?;
                    if base.is_some() || signed.is_some() || long_flag || short_flag { return Err(ParseError { msg: "'void' ne se combine pas avec signed/unsigned/short/long".into(), span: t.span }); }
                ...
                K::Float => {
                    let t = self.ts.bump().map_err(|e| ParseError { msg: e.msg, span: e.span })?;
                    if base.is_some() { return Err(ParseError { msg: "spécificateur de type en double".into(), span: t.span }); }
                    if signed.is_some() || short_flag || long_flag { return Err(ParseError { msg: "'float' ne se combine pas avec signed/unsigned/short/long".into(), span: t.span }); }
                ...
                K::Double => {
                    let t = self.ts.bump().map_err(|e| ParseError { msg: e.msg, span: e.span })?;
                    if base.is_some() { return Err(ParseError { msg: "spécificateur de type en double".into(), span: t.span }); }
                    if signed.is_some() || short_flag { return Err(ParseError { msg: "'double' ne se combine pas avec signed/unsigned/short".into(), span: t.span }); }
        ...
        let (ty, sp) = if let Some((ty, sp)) = base {
            ...
            if signed.is_some() || long_flag || short_flag {
                ...
                if short_flag && long_flag { return Err(ParseError { msg: "'short' et 'long' ne se combinent pas".into(), span: sp }); }
                (CType::Int { signed, long: long_flag, short: short_flag }, sp)
            } else {
                return Err(ParseError { msg: "spécificateur de type attendu (int/char/void/float/double)".into(), span: self.ts.peek(0).map_err(|e| ParseError { msg: e.msg, span: e.span })?.span });
            }
        };
        Ok((ty, sp))
    }
    ...
    pub(crate) fn is_type_start(&mut self) -> PResult<bool> {
        let t0 = self.ts.peek(0).map_err(|e| ParseError { msg: e.msg, span: e.span })?.clone();
    ...
    pub(crate) fn parse_type_name(&mut self) -> PResult<CType> {
        ...
        let ty = if self.ts.matches(K::LParen).map_err(|e| ParseError { msg: e.msg, span: e.span })? {
    ...
    pub(crate) fn parse_declaration_many(&mut self) -> PResult<Vec<Decl>> {
        ...
        loop {
            let t0 = self
                .ts
                .peek(0)
                .map_err(|e| ParseError { msg: e.msg.clone(), span: e.span })?
                .clone();
            match t0.kind {
                K::Typedef => { if storage.is_some() { return Err(ParseError { msg: "storage-class multiple".into(), span: t0.span }); } let _=self.ts.bump(); storage=Some(StorageClass::Typedef); }
                K::Extern => { if storage.is_some() { return Err(ParseError { msg: "storage-class multiple".into(), span: t0.span }); } let _=self.ts.bump(); storage=Some(StorageClass::Extern); }
                K::Static => { if storage.is_some() { return Err(ParseError { msg: "storage-class multiple".into(), span: t0.span }); } let _=self.ts.bump(); storage=Some(StorageClass::Static); }
                K::Auto => { if storage.is_some() { return Err(ParseError { msg: "storage-class multiple".into(), span: t0.span }); } let _=self.ts.bump(); storage=Some(StorageClass::Auto); }
                K::Register => { if storage.is_some() { return Err(ParseError { msg: "storage-class multiple".into(), span: t0.span }); } let _=self.ts.bump(); storage=Some(StorageClass::Register); }
                K::Const => { let _=self.ts.bump(); if quals.contains(&TypeQualifier::Const) { return Err(ParseError { msg: "qualifier 'const' en double".into(), span: t0.span }); } quals.push(TypeQualifier::Const); }
                K::Volatile => { let _=self.ts.bump(); if quals.contains(&TypeQualifier::Volatile) { return Err(ParseError { msg: "qualifier 'volatile' en double".into(), span: t0.span }); } quals.push(TypeQualifier::Volatile); }
                _ => break,
            }
        }
        ...
        loop {
            ...
            let init = if self.ts.matches(K::Assign).map_err(|e| ParseError { msg: e.msg, span: e.span })? { Some(self.parse_initializer()?) } else { None };
            decls.push(Decl { name, ty, init, storage, qualifiers: quals.clone(), bit_width: None, span: name_sp });
            if !self.ts.matches(K::Comma).map_err(|e| ParseError { msg: e.msg, span: e.span })? { break; }
        }
        let _ = self.ts.expect_kind(K::Semicolon).map_err(|e| ParseError { msg: e.msg, span: e.span })?;
        Ok(decls)
    }
    ...
    pub(crate) fn parse_top(&mut self) -> PResult<Top> {
        ...
        if self.ts.check(K::LBrace).map_err(|e| ParseError { msg: e.msg, span: e.span })? {
            let params = match &ty { CType::Function { params, .. } => params.clone(), _ => return Err(ParseError { msg: "définition: déclarateur de fonction attendu".into(), span: name_sp }) };
        ...
        let init = if self.ts.matches(K::Assign).map_err(|e| ParseError { msg: e.msg, span: e.span })? { Some(self.parse_initializer()?) } else { None };
        decls.push(Decl { name, ty, init, storage, qualifiers: quals.clone(), bit_width: None, span: name_sp });
        while self.ts.matches(K::Comma).map_err(|e| ParseError { msg: e.msg, span: e.span })? {
            let (n2, sp2, ty2) = self.parse_declarator_full(base_ty.clone())?;
            let init2 = if self.ts.matches(K::Assign).map_err(|e| ParseError { msg: e.msg, span: e.span })? { Some(self.parse_initializer()?) } else { None };
    ...
    fn apply_pointer_prefixes(&mut self, mut ty: CType) -> PResult<CType> {
        ...
        loop {
            if !self.ts.matches(K::Star).map_err(|e| ParseError { msg: e.msg, span: e.span })? { break; }
            let mut pquals: Vec<TypeQualifier> = Vec::new();
            loop {
                let t0 = self.ts.peek(0).map_err(|e| ParseError { msg: e.msg.clone(), span: e.span })?.clone();
                match t0.kind {
                    K::Const => { let _=self.ts.bump(); if pquals.contains(&TypeQualifier::Const) { return Err(ParseError { msg: "qualifier 'const' en double sur pointeur".into(), span: t0.span }); } pquals.push(TypeQualifier::Const); }
                    K::Volatile => { let _=self.ts.bump(); if pquals.contains(&TypeQualifier::Volatile) { return Err(ParseError { msg: "qualifier 'volatile' en double sur pointeur".into(), span: t0.span }); } pquals.push(TypeQualifier::Volatile); }
                    _ => break,
                }
            }
    ...
    fn parse_declarator_full(&mut self, base: CType) -> PResult<(String, Span, CType)> {
        ...
        let (name, sp) = if t0.kind == K::Ident {
            ...
            return Err(ParseError { msg: "identifiant ou déclarateur attendu".into(), span: t0.span });
        };
        // suffixes: fonctions/arrays (inclure K&R ident-lists)
        loop {
            if self.ts.matches(K::LParen).map_err(|e| ParseError { msg: e.msg, span: e.span })? {
                // K&R old-style: ident list (a, b, c)
                let t1 = self.ts.peek(0).map_err(|e| ParseError { msg: e.msg.clone(), span: e.span })?.clone();
                if t1.kind == K::RParen {
                    let _ = self.ts.bump();
            ...
            } else if self.ts.matches(K::LBracket).map_err(|e| ParseError { msg: e.msg, span: e.span })? {
                let mut sz: Option<usize> = None;
                let t0 = self.ts.peek(0).map_err(|e| ParseError { msg: e.msg, span: e.span })?;
                if t0.kind == K::IntLit { let tnum = self.ts.bump().map_err(|e| ParseError { msg: e.msg, span: e.span })?; if let Ok(v) = tnum.lexeme.parse::<usize>() { sz = Some(v); } }
                let _ = self.ts.expect_kind(K::RBracket).map_err(|e| ParseError { msg: e.msg, span: e.span })?;
    ...
    fn parse_param_list(&mut self) -> PResult<(Vec<Param>, bool, bool)> {
        ...
        let prototype = true;
        if self.ts.matches(K::RParen).map_err(|e| ParseError { msg: e.msg, span: e.span })? { return Ok((params, varargs, false)); }
        if self.ts.matches(K::Void).map_err(|e| ParseError { msg: e.msg, span: e.span })? { self.ts.expect_kind(K::RParen).map_err(|e| ParseError { msg: e.msg, span: e.span })?; return Ok((params, varargs, true)); }
        loop {
            if self.ts.matches(K::Ellipsis).map_err(|e| ParseError { msg: e.msg, span: e.span })? { varargs = true; self.ts.expect_kind(K::RParen).map_err(|e| ParseError { msg: e.msg, span: e.span })?; break; }
            ...
            if self.ts.matches(K::Comma).map_err(|e| ParseError { msg: e.msg, span: e.span })? { continue; }
            self.ts.expect_kind(K::RParen).map_err(|e| ParseError { msg: e.msg, span: e.span })?; break;
        }
    ...
    fn parse_initializer(&mut self) -> PResult<Initializer> {
        ...
        if self.ts.matches(K::LBrace).map_err(|e| ParseError { msg: e.msg, span: e.span })? {
            let mut elems = Vec::new();
            if !self.ts.check(K::RBrace).map_err(|e| ParseError { msg: e.msg, span: e.span })? {
                loop {
                    elems.push(self.parse_initializer()?);
                    if self.ts.matches(K::Comma).map_err(|e| ParseError { msg: e.msg, span: e.span })? {
                        if self.ts.check(K::RBrace).map_err(|e| ParseError { msg: e.msg, span: e.span })? { break; }
                        continue;
                    }
                    break;
                }
            }
            let _ = self.ts.expect_kind(K::RBrace).map_err(|e| ParseError { msg: e.msg, span: e.span })?;
    ...
    fn try_parse_tag_type(&mut self) -> PResult<Option<(CType, Span)>> {
        ...
        let tkw = self.ts.bump().map_err(|e| ParseError { msg: e.msg, span: e.span })?;
        let mut tag: Option<String> = None;
        if self.ts.check(K::Ident).map_err(|e| ParseError { msg: e.msg, span: e.span })? { let (n, _) = self.parse_ident()?; tag = Some(n); }
            ...
            if tkw.kind == K::Enum {
                ...
                if !self.ts.check(K::RBrace).map_err(|e| ParseError { msg: e.msg, span: e.span })? {
                    loop {
                        ...
                        let val = if self.ts.matches(K::Assign).map_err(|e| ParseError { msg: e.msg, span: e.span })? { 
                ...
                while !self.ts.check(K::RBrace).map_err(|e| ParseError { msg: e.msg, span: e.span })? {
                    let (field_base, _sp) = self.parse_type_specifier()?;
                    // Unnamed bit-field: ':' width;
                    if self.ts.matches(K::Colon).map_err(|e| ParseError { msg: e.msg, span: e.span })? {
                    ...
                    loop {
                        ...
                        // bit-field optionnel
                        let bw = if self.ts.matches(K::Colon).map_err(|e| ParseError { msg: e.msg, span: e.span })? { Some(self.parse_expr()?) } else { None };
                        // interdiction d'initialiseur dans les champs
                        if self.ts.matches(K::Assign).map_err(|e| ParseError { msg: e.msg, span: e.span })? {
                            return Err(ParseError { msg: "initialiseur de champ 'struct/union' interdit en C89".into(), span: fsp });
                        }
        ...
        if let Ok(t) = self.ts.peek(lookahead) {
            if t.kind == K::Semicolon || t.kind == K::LBrace {
                ...
                if let Some((ty, sp)) = self.try_parse_tag_type()? {
                    if self.ts.matches(K::Semicolon).map_err(|e| ParseError { msg: e.msg, span: e.span })? {
